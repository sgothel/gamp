...
class GearsObjectES2 {
  ...
  private:
    ...
    void vert(GLFloatArrayDataServerRef& array, float x, float y, float z, float n[]) {
        array->put3f(x, y, z);
  ...
  public:
    ...
    : m_st(st), m_pmvMatrix(pmvMatrix), m_pmvMatrixUniform(std::move(pmvMatrixUniform)),
      m_colorUniform(std::move(colorUniform)), m_gearColor(gearColor),
    ...
    // Unrotated object space PointerEventAction!
    bool runInObjSpace(float x, float y, const PointerEventAction& action, const PointerEvent& e) {
        m_pmvMatrix.pushMv();
        m_pmvMatrix.translateMv(x, y, 0.0f);
...
class GearsES2 : public RenderListener {
    ...
    class MyPointerListener : public PointerListener {
        ...
        bool navigate(const PointerEvent& e, GearsObjectES2& shape) noexcept {
            ...
            if( e.isControlDown() ) {
                if( m_dragInit ) {
                    ...
                    jau::math::Vec3f diffPos = objPos - startObjPos;
                    m_parent.m_panX += diffPos.x; // positive -> left
                    m_parent.m_panY += diffPos.y; // positive -> up
                }
            } else {
      ...
      public:
        ...
        void pointerPressed(const PointerEvent& e) override {
            if( e.pointerCount() == 1 ) {
                GearsObjectES2* new_pick = m_parent.runInObjSpace(pickAction, e);
                if( m_picked ) {
                    m_picked->picked() = false;
                }
                m_picked = new_pick;
        ...
        void pointerDragged(const PointerEvent& e) override {
            if( m_picked ) {
                GearsObjectES2* new_pick = m_parent.runInObjSpace(navigateAction, e);
                if( !new_pick ) {
                    if( m_picked ) {
                        m_picked->picked() = false;
                    }
  ...
  public:
    ...
    : RenderListener(RenderListener::Private()),
      ...
      m_sbox()
    {
        m_sbox.resize(m_gear1.bounds());
        m_sbox.resize(m_gear2.bounds());
        m_sbox.resize(m_gear3.bounds());
    }
    ...
    GearsObjectES2* runInObjSpace(const PointerEventAction& action, const PointerEvent& e) {
        m_pmvMatrix.pushMv();
        m_pmvMatrix.translateMv(m_panX, m_panY, m_panZ);
        m_pmvMatrix.rotateMv(jau::adeg_to_rad(m_view_rotx), 1.0f, 0.0f, 0.0f);
        m_pmvMatrix.rotateMv(jau::adeg_to_rad(m_view_roty), 0.0f, 1.0f, 0.0f);
        ...
        // We do not perform object rotatation in object space for PointerEventAction!
        GearsObjectES2* res = nullptr;
        if( m_gear1.runInObjSpace(-3.0f, -2.0f, action, e) ) {
            res = &m_gear1;
        }
        if( !res && m_gear2.runInObjSpace( 3.1f, -2.0f, action, e) ) {
             res = &m_gear2;
        }
        if( !res && m_gear3.runInObjSpace(-3.1f,  4.2f, action, e) ) {
            res = &m_gear3;
        }
        m_pmvMatrix.popMv();
        return res;
